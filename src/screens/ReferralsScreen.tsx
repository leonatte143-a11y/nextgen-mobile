...
export function ReferralsScreen() {
  ...
  const { user } = useAuth();
  const code = user?.referralCode ?? 'NEXGEN2026';

  const copy = () => {
    Alert.alert('Referral code', `${code}\n\n(Long-press to copy from alert — wire Clipboard when needed.)`);
  };
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.body}>
      ...
      <View style={styles.codeBox}>
        ...
        <PrimaryButton title="Copy" variant="outline" onPress={copy} />
      </View>
      <View style={styles.row2}>
        ...
        <View style={styles.cell}>
          <Text style={styles.big}>₹2,500</Text>
      ...
      <Text style={styles.rule}>• No limit on invites.</Text>
    </ScrollView>
  );
}
